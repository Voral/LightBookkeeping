...
class WhereGroupAndTest extends TestCase
{
	...
	public function getDataProvider()
	{
		...
		$where1 = new WhereGroupAnd([
			new Equal($fieldId, 2),
			new Like($fieldName,'test')
		]);
		$where2 = new WhereGroupAnd([new Equal($fieldId, 1)]);
		return [
			'single' => [
				$where2->get(),
				'a.id=1'
			...
			'add' => [
				...
				'(a.id<>10 and not a.name is null)'
			],
			'multiLevel' => [
				$where2->addItem($where1)->get(),
				'(a.id=1 and (a.id<>10 and not a.name is null))'
			],
			'empty' => [
				$where1->setItems([])->get(),
				''
